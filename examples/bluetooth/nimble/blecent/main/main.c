...
blecent_should_connect(const struct ble_gap_disc_desc *disc)
{
    ...
    if (strlen(CONFIG_PEER_ADDR) && (strncmp(CONFIG_PEER_ADDR, "ADDR_ANY", strlen("ADDR_ANY")) != 0)) {
        ESP_LOGI(tag, "Peer address from menuconfig: %s", CONFIG_PEER_ADDR);
        /* Convert string to address */
        sscanf(CONFIG_PEER_ADDR, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
